...
const ToastCard: React.FC<{ toast: Toast; onDismiss: () => void }> = ({ toast, onDismiss }) => (
  <button
    type="button"
    onClick={onDismiss}
    className={`animate-slide-in-right text-left bg-app-surface border border-line-subtle px-4 py-3 rounded-2xl shadow-float text-sm font-medium cursor-pointer hover:brightness-105 transition ${TONE_STYLES[toast.tone]}`}
  >
    {toast.message}
  </button>
);
