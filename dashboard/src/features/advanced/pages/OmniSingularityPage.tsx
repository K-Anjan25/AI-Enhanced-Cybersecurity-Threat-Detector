...
export default function OmniSingularityPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold">Omni-Singularity P121-P130 — Planetary & Interplanetary Defense</h1>
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ["inter","P121 Interplanetary"],
          ["agi","P122 AGI Council"],
          ["leg","P123 Legislation"],
          ["syn","P124 Synthetic"],
          ["holo","P125 Holographic"],
          ["wf","P126 Workforce"],
          ["con","P127 Consciousness"],
          ["planet","P128 Planetary"],
          ["time","P129 Time Prophecy"],
          ["omni","P130 Omni-OS"],
        ].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k as any)} className={`px-3 py-1 rounded border text-xs ${tab===k?'bg-indigo-600 text-white border-indigo-600':'bg-white'}`}>{l}</button>
        ))}
      </div>

      {tab==="inter" && (
        <div className="space-y-3">
          <button onClick={createNode} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Interplanetary Node (satellite LEO 20ms)</button>
          <div className="grid gap-2">{nodes.map((n:any)=>(<div key={n.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{n.node_name} {n.node_type} {n.location} latency {n.latency_ms}ms bw {n.bandwidth_mbps}Mbps {n.status}</div></div><button onClick={()=>ingestTele(n.id)} className="px-2 py-1 bg-orange-600 text-white rounded">Ingest Anomaly Telemetry (DTN bundle)</button></div>))}</div>
        </div>
      )}

      {tab==="agi" && (
        <div className="space-y-3">
          <button onClick={createCouncil} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create AGI Council (Athena Sentinel Oracle Guardian Sage)</button>
          <div className="grid gap-2">{councils.map((c:any)=>(<div key={c.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{c.name} {c.council_type} quorum {c.quorum_required} {c.consensus_strategy}</div></div><button onClick={()=>convene(c.id)} className="px-2 py-1 bg-violet-600 text-white rounded">Convene Council (supermajority vote)</button></div>))}</div>
        </div>
      )}

      {tab==="leg" && (
        <div className="space-y-3">
          <button onClick={createReg} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Regulation Source (GDPR Article 33 72h)</button>
          <div className="grid gap-2">{regs.map((r:any)=>(<div key={r.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{r.name} {r.framework} v{r.version}</div></div><button onClick={()=>genPolicy(r.id)} className="px-2 py-1 bg-green-600 text-white rounded">Generate OPA Rego Policy</button></div>))}</div>
        </div>
      )}

      {tab==="syn" && (
        <div className="space-y-3">
          <button onClick={createUni} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Synthetic Universe (large 100k realism 92% privacy)</button>
          <div className="grid gap-2">{universes.map((u:any)=>(<div key={u.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{u.name} {u.universe_type} {u.scale} realism {u.realism_score} privacy {String(u.privacy_preserved)}</div></div><button onClick={()=>genDataset(u.id)} className="px-2 py-1 bg-violet-600 text-white rounded">Generate Dataset GAN+LLM 10k alerts fidelity 0.92</button></div>))}</div>
        </div>
      )}

      {tab==="holo" && (
        <div className="space-y-3">
          <button onClick={createDisplay} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Holographic Display (volumetric 8K 85 inch war room)</button>
          <div className="grid gap-2">{displays.map((d:any)=>(<div key={d.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{d.display_name} {d.display_type} {d.resolution} {d.size_inches} inch {d.location}</div></div><button onClick={()=>createHolo(d.id)} className="px-2 py-1 bg-fuchsia-600 text-white rounded">Spawn Threat Globe Hologram (earth lat/lon HIGH)</button></div>))}</div>
        </div>
      )}

      {tab==="wf" && (
        <div className="space-y-3">
          <button onClick={createWf} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create AI Workforce (25 agents 5 human 20 AI 80% autonomy)</button>
          <div className="grid gap-2">{workforces.map((w:any)=>(<div key={w.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{w.name} {w.workforce_type} total {w.total_agents} human {w.human_count} AI {w.ai_count} autonomy {(w.autonomy_ratio*100).toFixed(0)}%</div></div><button onClick={()=>assignTask(w.id)} className="px-2 py-1 bg-green-600 text-white rounded">Assign Hunt Task to hunter-agent</button></div>))}</div>
        </div>
      )}

      {tab==="con" && (
        <div className="space-y-3">
          <button onClick={createCon} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Consciousness Profile (claude consciousness 12.5 self-awareness 15)</button>
          <div className="grid gap-2">{conProfiles.map((p:any)=>(<div key={p.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{p.ai_agent_name} {p.model} consciousness {p.consciousness_score} self {p.self_awareness} align {p.alignment_score} corrig {p.corrigibility_score} {p.status}</div></div><button onClick={()=>checkAlign(p.id)} className="px-2 py-1 bg-yellow-600 text-white rounded">Run Alignment Check (helpfulness harmlessness honesty)</button></div>))}</div>
        </div>
      )}

      {tab==="planet" && (
        <div className="space-y-3">
          <button onClick={createGrid} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Planetary Defense Grid (global power/water/telecom/finance/healthcare readiness 87.5%)</button>
          <div className="grid gap-2">{grids.map((g:any)=>(<div key={g.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{g.name} {g.grid_type} threat {g.threat_level} readiness {g.defense_readiness}%</div><div>Coverage {JSON.stringify(g.coverage)}</div></div><button onClick={()=>createThreat(g.id)} className="px-2 py-1 bg-red-600 text-white rounded">Create Nation-State Threat (impact 85)</button></div>))}</div>
        </div>
      )}

      {tab==="time" && (
        <div className="space-y-3">
          <button onClick={createTemporal} className="px-3 py-1 bg-indigo-600 text-white rounded text-xs">Create Temporal Model (transformer hourly 90d lookback 30d forecast acc 0.89)</button>
          <div className="grid gap-2">{temporalModels.map((m:any)=>(<div key={m.id} className="border p-3 rounded bg-white text-xs flex justify-between"><div><div className="font-bold">{m.name} {m.model_type} {m.time_granularity} lookback {m.lookback_days}d forecast {m.forecast_horizon_days}d acc {m.accuracy}</div></div><button onClick={()=>prophesy(m.id)} className="px-2 py-1 bg-violet-600 text-white rounded">Prophesy Breach (causal graph root cause failed_logins)</button></div>))}</div>
        </div>
      )}

      {tab==="omni" && (
        <div className="space-y-3">
          {omniConfig && <div className="border p-3 rounded bg-white text-xs"><div className="font-bold text-xl">NOCTRA Omni-OS v3 {omniConfig.version} {omniConfig.omnipresence_level}</div><div>Targets {omniConfig.deployment_targets?.join(", ")} consciousness {String(omniConfig.consciousness_enabled)} self_awareness {omniConfig.self_awareness_level} status {omniConfig.status}</div></div>}
          {omniMetrics && <div className="border p-3 rounded bg-white text-xs"><div className="font-bold">Omni Metrics</div><pre className="text-[10px]">{JSON.stringify(omniMetrics,null,2)}</pre></div>}
          <div className="border p-2 rounded bg-white text-xs"><div className="font-bold">Omni Nodes (exists everywhere)</div>{omniNodes.map((n:any)=>(<div key={n.id}>{n.node_name} {n.node_type} {n.location} compute {n.compute_units} autonomous {String(n.is_autonomous)} {n.status}</div>))}</div>
          <div className="border p-6 rounded bg-gradient-to-r from-black via-indigo-900 to-violet-900 text-white">
