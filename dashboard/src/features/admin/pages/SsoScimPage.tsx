...
export default function SsoScimPage(): React.ReactElement {
  ...
  const load = async () => {
    ...
    try {
      ...
      const org = sso.org?.enabled ? sso.org : sso.global?.enabled ? sso.global : null;
      if (org) {
        setSsoEnabled(true);
        setSsoIssuer(org.issuer || "");
        setSsoClientId(org.client_id || "");
        setSsoDisplayName(org.display_name || "Corporate SSO");
        setSsoScopes(org.scopes || "openid email profile");
        setSsoJit(org.jit ?? true);
      }
  ...
  const handleSaveSso = async () => {
    ...
    try {
      if (!ssoEnabled) {
        ...
        showSuccess("SSO provider disabled");
        await load();
        return;
      }
      if (!ssoIssuer || !ssoClientId) {
        setError("Issuer and Client ID are required");
        setSaving(false);
        return;
      }
      ...
      showSuccess("SSO provider saved");
      setSsoClientSecret("");
      await load();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="SSO & SCIM"
        description="Configure OIDC single sign-on and SCIM provisioning for enterprise identity providers (Okta, Entra ID, Google). SAML is not implemented yet — documented as gap."
      />
      ...
      {lastCreatedToken && (
        <div className="px-4 py-3 rounded-lg bg-status-warning/10 border border-status-warning/30 text-sm">
          ...
          <p className="mt-2 text-xs text-content-tertiary">Use as Bearer token for SCIM: Authorization: Bearer &lt;token&gt; → /scim/v2/Users</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* SSO */}
        <Card padded>
          <h3 className="text-sm font-bold text-content-primary mb-1">Single Sign-On (OIDC)</h3>
          <p className="text-xs text-content-tertiary mb-4">OIDC Authorization Code flow with JIT provisioning. Users are created on first login with role USER/ANALYST (never ADMIN).</p>

          <div className="space-y-3">
            <label className="flex items-center gap-2 text-xs">
              <input type="checkbox" checked={ssoEnabled} onChange={(e) => setSsoEnabled(e.target.checked)} />
              <span className="font-semibold">Enable SSO</span>
            </label>

            <div>
              <label className="text-[11px] font-bold uppercase tracking-wider text-content-tertiary">Display Name</label>
              <input className="mt-1 w-full px-3 py-2 rounded-lg bg-app-subtle border border-line-subtle text-sm" value={ssoDisplayName} onChange={(e) => setSsoDisplayName(e.target.value)} placeholder="Corporate SSO" />
            </div>
            ...
            <label className="flex items-center gap-2 text-xs">
              ...
              <span>JIT provisioning — create user on first SSO login</span>
            </label>

            <Button variant="primary" size="sm" onClick={handleSaveSso} disabled={saving}>{saving ? "Saving…" : ssoEnabled ? "Save SSO Provider" : "Disable SSO"}</Button>

            <p className="text-[10px] text-content-tertiary">Callback URL to configure in IdP: <code className="bg-app-subtle px-1 rounded">/api/v1/auth/sso/callback</code></p>
          </div>
        </Card>

        {/* SCIM */}
        <Card padded>
          <h3 className="text-sm font-bold text-content-primary mb-1">SCIM 2.0 Provisioning</h3>
          <p className="text-xs text-content-tertiary mb-4">Bearer token auth. Endpoints: /scim/v2/Users, /Groups (minimal), /ServiceProviderConfig, /ResourceTypes, /Schemas. Tokens are per-org, hashed at rest.</p>
          ...
          <div className="space-y-3">
            ...
            <div className="pt-3 border-t border-line-subtle space-y-1">
              ...
              <p className="text-[11px] text-content-secondary">Auth: Bearer token (the value shown once on creation)</p>
              <p className="text-[11px] text-content-secondary">Supported: Users CRUD + filter userName/email eq, Groups list minimal, PATCH active</p>
              <p className="text-[10px] text-content-tertiary">Groups membership sync not implemented — documented as gap. Bulk extension not implemented.</p>
            </div>
          </div>
        </Card>
      </div>
